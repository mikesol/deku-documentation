...
performanceConsiderations = subsection
  ...
  , matter: \(Env { routeLink }) ->
      [ D.p_
          ...
          , D.i__ "any"
          , text_
              " type. Hooks can even contain Deku components, which means that a component can be pushed to a pusher and sent to an event. For this reason, once a global portal is created, it can theoretically be injected anywhere in an application and thus must be kept alive for the lifespan of the applications. In many scenarios, for example an in-app chat widget, this is fine. But in many scenarios, like if a portal exists in a list element, you'll want the portal to be cleaned up when the element is cleaned up. For that, we have "
          , D.b__ "local portals"
          , text_
