...
theGlobalPortalSyntax = subsection
  ...
  , matter: pure
      ...
      , D.p_
          [ text_ "While using a portal looks like using any ol' Deku node, don't let it fool you! Portals are created with a left bind, and as such are referentially opaque. That means that, when a portal is created, it represents "
          , D.i__ "a single DOM node that exists"
          , text_ ", as opposed to a template for a DOM node."
          ]
        , D.p__ "Relating this concept to the example above, let's zoom in on the following line of code:"
        , psCode "vid <- globalPortal1 myVideo"
        , D.p_[ text_ "The term ", D.code__ "vid", text_ " is one single instantiation of our video. On the other hand, if we had passed ", D.code__ "myVideo", text_ " to the ", D.code__ "moveSpriteHere", text_ " function, we would be working with a referentially transparent object that acts as a blueprint for a node to construct as opposed to an actual node."]
        , D.p__ "Because DOM nodes can only appear in one place in the DOM, when a portal is used multiple times, the behavior is undefined. For that reason, in your application logic, it is important to make sure that a portal only ever appears in at most one place."
      ]
  }
